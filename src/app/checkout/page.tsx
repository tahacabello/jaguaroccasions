...
import { supabase, getSupabaseSettings } from "@/lib/supabase";
import { sendTelegramNotification, buildOrderMessage, saveNotificationToStorage } from "@/lib/telegram";
...
export default function CheckoutPage() {
  ...
  const { cartItems, cartTotal, cartSubtotal, discount, applyCoupon, clearCart, isLoaded } = useCart();
  ...
  // Load Telegram configuration from settings on mount
  useEffect(() => {
    ...
    }).catch(err => console.error("Error reading Telegram settings inside checkout page:", err));
  }, []);
  ...
  const handleApplyCoupon = (e: React.FormEvent) => {
    ...
    const result = applyCoupon(localCouponInput);
    if (result.success) {
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Send Telegram notification to admin's phone
      try {
        const tgMessage = buildOrderMessage({
          trackingNumber: orderData.tracking_number,
          customerName: orderData.guest_name,
          phone: orderData.guest_phone,
          city: orderData.guest_city,
          address: orderData.guest_address,
          total: orderData.total_amount,
          shippingFee: orderData.shipping_fee,
          paymentMethod: orderData.payment_method,
          items: cartItems.map(item => ({ name: item.name, quantity: item.quantity, price: item.price, mode: item.mode })),
        });

        const tgSent = await sendTelegramNotification(telegramToken, telegramChatId, tgMessage);

        // Save notification to localStorage for admin dashboard
        saveNotificationToStorage({
          id: `notif-${Date.now()}`,
          orderId: orderData.tracking_number,
          customerName: orderData.guest_name,
          city: orderData.guest_city,
          total: orderData.total_amount,
          paymentMethod: orderData.payment_method,
          status: tgSent ? "sent" : "failed",
          timestamp: new Date().toISOString(),
          message: tgMessage,
        });
      } catch (tgErr) {
        // Telegram error must NEVER block the order success
        console.warn("Telegram notification failed (non-blocking):", tgErr);
      }
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="glass p-6 rounded-3xl border border-border space-y-6 sticky top-24">
                ...
                <form onSubmit={handleApplyCoupon} className="pt-4 border-t border-border space-y-2">
                  ...
                  {couponMsg.type === "error" && <p className="text-xs text-red-400 font-bold mt-1">{couponMsg.text}</p>}
                  {couponMsg.type === "success" && <p className="text-xs text-green-400 font-bold mt-1">{couponMsg.text}</p>}
                </form>
